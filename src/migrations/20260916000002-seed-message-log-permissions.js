...
const MESSAGE_LOG_PERMISSIONS = [
  { permission_key: 'message_logs.read', description: 'Sistem SMS / e-posta kayıtlarını görüntüleme' },
];

const ROLE_KEYS = {
  Müdür: ['message_logs.read'],
  'Müdür Yardımcısı': ['message_logs.read'],
};
...
module.exports = {
  async up(queryInterface) {
    ...
    const existing = await selectAll(
      queryInterface,
      `SELECT permission_key FROM permissions WHERE permission_key LIKE 'message_logs.%'`
    );
    const existingKeys = new Set(existing.map((r) => r.permission_key));
    const missing = MESSAGE_LOG_PERMISSIONS.filter((p) => !existingKeys.has(p.permission_key));
    ...
    const permissions = await selectAll(
      queryInterface,
      `SELECT id, permission_key FROM permissions WHERE permission_key LIKE 'message_logs.%'`
    );
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(
      `DELETE FROM role_permissions WHERE permission_id IN (
         SELECT id FROM permissions WHERE permission_key LIKE 'message_logs.%'
       )`
    );
    await queryInterface.bulkDelete('permissions', {
      permission_key: MESSAGE_LOG_PERMISSIONS.map((p) => p.permission_key),
    });
  },
};
