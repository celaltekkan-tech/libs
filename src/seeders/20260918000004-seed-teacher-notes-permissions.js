...
const PERMISSIONS = [
  { permission_key: 'teacher_notes.create', description: 'Öğretmen bildirimi oluşturma (mobil uygulama)' },
];

const ROLE_KEYS = {
  'Öğretmen': ['teacher_notes.create'],
  'Müdür': ['teacher_notes.create'],
  'Müdür Yardımcısı': ['teacher_notes.create'],
};
...
module.exports = {
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(
      `DELETE FROM role_permissions WHERE permission_id IN (SELECT id FROM permissions WHERE permission_key LIKE 'teacher_notes.%')`
    );
