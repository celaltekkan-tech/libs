...
const PERMISSIONS = [
  { permission_key: 'duty.read', description: 'Nöbet programını listeleme' },
  { permission_key: 'duty.create', description: 'Nöbet ataması oluşturma' },
  { permission_key: 'duty.update', description: 'Nöbet ataması güncelleme' },
  { permission_key: 'duty.delete', description: 'Nöbet ataması silme' },
];

const ROLE_KEYS = {
  'Müdür': ['duty.read', 'duty.create', 'duty.update', 'duty.delete'],
  'Müdür Yardımcısı': ['duty.read', 'duty.create', 'duty.update', 'duty.delete'],
  'Memur': ['duty.read'],
  'Öğretmen': ['duty.read'],
};
...
module.exports = {
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(
      `DELETE FROM role_permissions WHERE permission_id IN (SELECT id FROM permissions WHERE permission_key LIKE 'duty.%')`
    );
