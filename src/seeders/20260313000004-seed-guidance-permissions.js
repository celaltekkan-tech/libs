...
// Gizlilik dereceli modül: yalnızca Müdür ve Rehber Öğretmen erişebilir.
const PERMISSIONS = [
  { permission_key: 'guidance.read', description: 'Rehberlik görüşme kayıtlarını listeleme' },
  { permission_key: 'guidance.create', description: 'Rehberlik görüşme kaydı oluşturma' },
  { permission_key: 'guidance.update', description: 'Rehberlik görüşme kaydı güncelleme' },
  { permission_key: 'guidance.delete', description: 'Rehberlik görüşme kaydı silme' },
];

const ROLE_KEYS = {
  'Müdür': ['guidance.read'],
  'Rehber Öğretmen': ['guidance.read', 'guidance.create', 'guidance.update', 'guidance.delete'],
};
...
module.exports = {
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(
      `DELETE FROM role_permissions WHERE permission_id IN (SELECT id FROM permissions WHERE permission_key LIKE 'guidance.%')`
    );
