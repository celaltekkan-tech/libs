...
const PERMISSIONS = [
  { permission_key: 'teacher_documents.read', description: 'Öğretmen evrak arşivini listeleme' },
  { permission_key: 'teacher_documents.create', description: 'Öğretmen evrakı oluşturma' },
  { permission_key: 'teacher_documents.update', description: 'Öğretmen evrakı güncelleme / teslim durumu değiştirme' },
  { permission_key: 'teacher_documents.delete', description: 'Öğretmen evrakı silme' },
];

const ROLE_KEYS = {
  'Müdür': ['teacher_documents.read', 'teacher_documents.create', 'teacher_documents.update', 'teacher_documents.delete'],
  'Müdür Yardımcısı': ['teacher_documents.read', 'teacher_documents.update'],
  'Öğretmen': ['teacher_documents.read', 'teacher_documents.create', 'teacher_documents.update'],
  'Rehber Öğretmen': ['teacher_documents.read', 'teacher_documents.create', 'teacher_documents.update'],
};
...
module.exports = {
  async up(queryInterface) {
    const now = new Date();
    const prefix = 'teacher_documents.%';
  ...
  async down(queryInterface) {
    await queryInterface.sequelize.query(
      `DELETE FROM role_permissions WHERE permission_id IN (SELECT id FROM permissions WHERE permission_key LIKE 'teacher_documents.%')`
    );
