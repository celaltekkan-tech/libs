...
const UPLOAD_ROOT = path.resolve(process.cwd(), 'uploads', 'schools');
...
const upload = multer({
  ...
  fileFilter(_req, file, cb) {
    if (!isAllowedFile(file)) {
      const err = new Error('Yalnızca PNG, JPG veya WEBP logo dosyaları yüklenebilir');
      err.status = 400;
...
function saveBuffer(schoolId, file) {
  ensureUploadDir();
  const ext = extensionOf(file.originalname) || '.jpg';
  const storedName = `school-${schoolId}-${Date.now()}-${crypto.randomBytes(6).toString('hex')}${ext}`;
